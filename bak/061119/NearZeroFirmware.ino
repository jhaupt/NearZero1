...
Todo: 
-Add hall support.
-Either get DYNAMIC mode working or delete it.
...
int power_2_low;
int j = 0;		//general purpose counter
...
float Iset1;    //Current draw in mA. Power is automatically ramped to this value at start time.
float Iset2; 
...
const int addr_gain1l = 4;
const int addr_gain1r = 5;
const int addr_gain2l = 6;
const int addr_gain2r = 7;
const int addr_torqueprofile1 = 8;
const int addr_torqueprofile2 = 9;
const int addr_currentmode1 = 10;
const int addr_currentmode2 = 11;
const int addr_commandmode1 = 12;
const int addr_commandmode2 = 13;
const int addr_sensortype1 = 14;
const int addr_sensortype2 = 15;
const int addr_minIset1l = 16;	
const int addr_minIset1r = 17;
const int addr_minIset2l = 18;
const int addr_minIset2r = 19;
const int addr_tscoeff1 = 20;
const int addr_tscoeff2 = 21;
const int addr_tsphase1l = 22;
const int addr_tsphase1r = 23;
const int addr_tsphase2l = 24;
const int addr_tsphase2r = 25;
const int addr_accel1l = 26;
const int addr_accel1r = 27;
const int addr_accel2l = 28;
const int addr_accel2r = 29;
...
void Roll_vel2(){
	...
	if (accel2 == 10000) {	//If accel=10000, this means the accel variable shouldn't even be used
		...
		if (wheel2_vel == 0){	//if the wheel isn't moving (is resting), set the power low as determined by the config settings
			power_2 == power_2_low;
		}
	}

	//CONSIDER HALL SENSOR INPUT
	//pin3 HIGH forwards 5.5 to 2.4, backwards -4.6 to -1.6
	//pin4 HIGH forwards 1.5 to 4.7, backwards -2.3 to -5.4
	//pin5 HIGH forwards 3.5 to 0.5, backwards -0.3 to -3.6
	if (sensortype2 == 2){		//If the sensor type is HALL
		if (phaseindex_2 > 5.5 && phaseindex_2 <= 6.28 || phaseindex_2 >= 0 && phaseindex_2 < 2){
			if (digitalRead(enc2_pin3)){	//Keep resetting actualvel until the Hall input for this third of phase indexes goes high, signifying the motor has caught up with the current commanded location.
				wheel2_actualvel =0;		//pause the index increment
			}
		}
		if (phaseindex_2 > 1.5 && phaseindex_2 <= 4.7){
			if (digitalRead(enc2_pin4)){	//Keep resetting actualvel until the Hall input for this third of phase indexes goes high, signifying the motor has caught up with the current commanded location.
				wheel2_actualvel =0;		//pause the index increment
			}
		}
		if (phaseindex_2 > 3.5 && phaseindex_2 <= 6.28 || phaseindex_2 >= 0 && phaseindex_2 < .5){
			if (digitalRead(enc2_pin5)){	//Keep resetting actualvel until the Hall input for this third of phase indexes goes high, signifying the motor has caught up with the current commanded location.
				wheel2_actualvel =0;		//pause the index increment
			}
		}
	}
	...
	if(fabs(phaseindex_2) >= 2*pi) {   //Reset phaseindex_U1 once it completes 2*180o in phase.
		phaseindex_2 = 0;
	}
