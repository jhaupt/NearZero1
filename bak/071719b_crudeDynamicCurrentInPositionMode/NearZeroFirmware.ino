...
-Tweak for dynamic support on other motors
...
void Roll_vel2(){
	...
	if (sensortype2 == 2){
		...
		if (currentmode2 == 0) {	
			...
			//INCREMENT PHASE INDEX
			if (dir2 == 0) {	//If direction is NORMAL
				phaseindex2 = phaseindex2 + ((20/(100*((phaselag2U+phaselag2V+phaselag2W)/3)))*actualvel2);    //This is fed into the sine function to vary the pwm duty cycle. rate is the step size for incrementing phaseindex_U1
			}
			else if (dir2 == 1) {	//If direction is REVERSE
				phaseindex2 = phaseindex2 - ((20/(100*((phaselag2U+phaselag2V+phaselag2W)/3)))*actualvel2);
			}		
		}
		//IF WE'RE IN 'DYNAMIC' CURRENT MODE
		else if (currentmode2 == 1) {
			power2 = power2_low + ((int) 100*((phaselag2U+phaselag2V+phaselag2W)/3));
...
void Roll_pos2(){
	...
	if (sensortype2 == 2){
		...
		if (currentmode2 == 0) {
			...
			if ((posindexproxy2 - .2) > pos2){		//the constant defines the deadzone
				posindexproxy2 = posindexproxy2 - servovel2;
				phaseindex2 = phaseindex2 - ((20/(10*((phaselag2U+phaselag2V+phaselag2W)/3)))*servovel2);  
				power2 = power2_high;
				j = 0;
			}
			else if ((posindexproxy2 + .2) < pos2){
