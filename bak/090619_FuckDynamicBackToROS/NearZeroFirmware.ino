...
-Add user settable switches for /odom and /i_draw and calibration for /odom
-Add servo ROS subscriber. Look at PWM function to see how vel or pos is used.
-Add ENCODER support
...
void Roll_vel2(){
	...
	if (sensortype2 == 2){
		...
		if (currentmode2 == 0) {	
			...
			//INCREMENT PHASE INDEX
			if (dir2 == 0) {	//If direction is NORMAL
				phaseindex2 = phaseindex2 + actualvel2;		//Scale actualvel2 by (vel2*sensdelay2_T)-phasevelconst2
			}
			else if (dir2 == 1) {	//If direction is REVERSE
				phaseindex2 = phaseindex2 - actualvel2;
			}	
		...
		//IF WE'RE IN 'DYNAMIC' CURRENT MODE
			...
			power2 = ((2*sensdelay2_velcomp)*power2_low)-280;		//IMPORTANT: The multiplier sets the sensitivity of the power scaling, and the constant that's subtracted sets the offset. They always need to be changed together (if doubling the multiplier, roughly double the offset).
