...
void PROMPT_rosname2Entry(){			
	addr_rosname2 = 70;	//starting EEPROM address for storing string
	SerialReceiveLoop();
	//rosname2 = rxchars;
	for (int i = 0; i <= nchars; i++) {
		EEPROM.put(addr_rosname2, rxchars[i]);
		rosname2[i] = rxchars[i];
		addr_rosname2++; 
	}
}
...
int SerialPrompt(){
	...
	if (rxchars[0] == '2'){	
		...
		Serial.println(F("2: Set PWM command type (VELOCITY or POSITION)"));
		Serial.println(F("3: Set ROS configuration"));
		...
		Serial.println(F("6: Set velocity/position GAIN"));
		...
		//**Set PWM command type to VELOCITY or POSITION
			...
			Serial.println(F("Set channel 2 to interpret PWM inputs as either velocity or position commands:"));
			Serial.println(F("   v: VELOCITY (default)"));
			Serial.println(F("   p: POSITION"));
			...
			if (rxchars[0] == 'v' || rxchars[0] == 'V'){
				...
				Serial.println(F("PWM COMMAND TYPE NOT CHANGED"));
			}	
			Serial.println(F("\n\n\n\n"));
			delay(2000);		
			goto ch2menu;
		...
		//**Set ROS configuration
			...
			Serial.println(F("2:  Configure topics and command type"));
			Serial.println(F("\n   (Or strike 'Enter' to leave as is)")); 
			Serial.println(F("\n\n>>"));
			SerialReceiveLoop();
			if (rxchars[0] == '1'){		
				...
				Serial.println(F("   e: Quadrature ENCODER -- publishes quadrature ticks (integers 0-3) on /enc topic."));
				Serial.println(F("   h: HALL -- publishes hall ticks (integers 0-2) on /enc topic"));	
				Serial.println(F("\n   (Or strike 'Enter' to leave as is)")); 
				Serial.println(F("\n\n>>"));	
				SerialReceiveLoop();
				if (rxchars[0] == 'n' || rxchars[0] == 'N'){
			...
			if (rxchars[0] == '2'){		
				Serial.println(F("Configure channel 2 ROS topic and command type:"));
				Serial.println(F("1:  Subscribe to /joint_command topic for velocity or position control (DEFAULT)"));
				Serial.println(F("2:  Subscribe to /cmd_vel directly for differential drive"));	
				Serial.println(F("\n   (Or strike 'Enter' to leave as is)")); 
				Serial.println(F("\n\n>>"));	
				SerialReceiveLoop();
				if (rxchars[0] == '1'){
					Serial.println(F("Enter 'v' to command by VELOCITY or 'p' to command by POSITION:")); 
					Serial.println(F("	(Or strike 'Enter' to leave as is)")); 	
					Serial.println(F("\n\n>>"));
					SerialReceiveLoop();
					if (rxchars[0] == 'v' || rxchars[0] == 'V'){
						Serial.print(F("CHANNEL 2 SUBSCRIBED TO '/joint_command' WITH VELOCITY INPUT"));
						rostopic2 = 0;
						EEPROM.write(addr_rostopic2, rostopic2);	//0 = joint_command in velocity mode
					}
					else if (rxchars[0] == 'p' || rxchars[0] == 'P'){
						Serial.print(F("CHANNEL 2 SUBSCRIBED TO '/joint_command' WITH POSITION INPUT"));
						rostopic2 = 1;
						EEPROM.write(addr_rostopic2, rostopic2);	//1 = joint_command in position mode
					}
					else {
					...
					Serial.println(F("   (Or strike 'Enter' to leave as is)")); 	
					Serial.println(F("\n\n>>"));
					PROMPT_rosname2Entry();
					if (exitflag == true){
						delay(2000);
						goto ch2menu;		//Yes that's a goto. FRACK modern programming sensibilities. 
					}
					Serial.print(F("CHANNEL 2 JOINT NAME SET TO '"));
					Serial.print(rosname2);
					Serial.print(F("'."));
					Serial.println(F("\n\n\n\n"));
					delay(2000);		
					goto ch2menu;
					...
					Serial.println(F("Subscribe to /cmd_vel for differential drive of channels 1 and 2?")); 
					Serial.println(F(" "));
					...
					Serial.println(F("encoder (or hall) ticks on the /enc topic and depends on ROS-side software (either the NearZero ROS"));
					...
					Serial.println(F("publishes to /joint_command for each motor using the motor names (numbers) defined in the config file."));
					Serial.println(F(" "));
					...
					if (rxchars[0] == 'Y' || rxchars[0] == 'y'){
						rostopic2 = 2;
						//EEPROM.write(addr_rostopic1, rostopic1);	//2 = cmd_vel
						EEPROM.write(addr_rostopic2, rostopic2);	//2 = cmd_vel
