...
void CheckConfig(){
	...
	//Read topicsub2
	rostopic2 = EEPROM.read(addr_rostopic2);
	if (rostopic2 == 255){
		rostopic2 = 0;		//0 = joint_command for velocity, 1 = joint_command for position, 2 = cmd_vel
	}

	//rosname2[0] = 'x';
	//rosname2[1] = 'y';
	//EEPROM.put(70, rosname2[0]);
	//EEPROM.put(71, rosname2[1]);
	//rosname[0] = EEPROM.read(70);  	//Read the character in the part of the array given by i into the address given by chaddr.
	//rosname[1] = EEPROM.read(71);  	//Read the character in the part of the array given by i into the address given by chaddr.
	//Serial.print(rosname[0]);
	//Serial.println(rosname[1]);

	//Read rosname2
	addr_rosname2 = 70;
	iholder = EEPROM.read(addr_rosname2);
	if (iholder == 255){
		rosname2[0] = 'r'; rosname2[1] = 'i'; rosname2[2] = 'g'; rosname2[3] = 'h'; rosname2[4] = 't'; rosname2[5] = '_'; rosname2[6] = 'w'; rosname2[7] = 'h'; rosname2[8] = 'e'; rosname2[9] = 'e'; rosname2[10] = 'l';
	}
	else {
		for (int i = 0; i <= nchars; i++) {
			rosname2[i] = EEPROM.read(addr_rosname2);  	//Read the character in the part of the array given by i into the address given by chaddr.
			addr_rosname2++; 
		}
	}
}
...
void DisplaySettings2(){
	...
	if (rostopic2 == 0){
		Serial.println(F("   ROS command mode: VELOCITY"));
		Serial.println(F("   ROS topic for command input: /joint_command"));
		Serial.print(F("   ROS joint name: "));
		Serial.println(rosname2);
	}
	else if (rostopic2 == 1){
		Serial.println(F("   ROS command mode: POSITION"));
		Serial.println(F("   ROS topic for command input: /joint_command"));
		Serial.print(F("   ROS joint name: "));
		Serial.println(rosname2);
	}
		...
		Serial.println(F("   ROS command mode: Differential drive"));
		Serial.println(F("   Wheel base scale: "));
		Serial.println(F("   Wheel diameter scale: "));
	}
	if (sensortype2 == 0){
