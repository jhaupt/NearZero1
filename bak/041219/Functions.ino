int CalcMotion(){	//Not used in PWM control mode.
	wheel1_vel = (v_x*vcal_1*.0025)+(v_th*.01);		//Convert v_x and v_th to wheel motion. The angular rate (V_th) is added for this wheel... 
	wheel2_vel = (v_x*vcal_2*.0025)-(v_th*.01);		//...and subtracted from this one.
}
...
void TorqueCal(){ //Drive the motor a certain amount for the purpose of capturing velocity.
	do {
		wheel1_vel = .003;
		wheel2_vel = 0;

		if(phaseindex_U1 >= 6*pi){   //Disable the drive after a complete commutation cycle. 
...
int CheckMode(){
	...
	if((digitalRead(mode_usb) == HIGH) && (digitalRead(mode_i2c) == HIGH) && (digitalRead(mode_config) == HIGH))  {     //If none of the mode pins are LOW, read the PWM inputs to control each motor's velocity independently.
		...
		if(pwm1active == true){     //Only get velocity data from this PWM channel if it was found to be connected above
			...
			wheel1_vel = (pwm1_throttle-1500)*.001*vcal_1; //Normal PWM servo pulsewidth goes from 800 to 2200ms. We subtract 1500ms, corresponding to the middle of that range, so that pulses in the range 800-1499 will have negative velocity, and 1501-2200 will be positive. 
		}

		if(pwm2active == true){    //Only get velocity data from this PWM channel if it was found to be connected above
			pwm2_throttle = pulseIn(pwm2_in, HIGH);
			wheel2_vel = (pwm2_throttle-1500)*.001*vcal_2;
		}
		pwmflag = true;
	}
}
