...
-Fine-tune quiescent current comparison in SetPower(). Check minimum permissible values for I_set_ variables. Should be able to go less than 100mA.
...
float pi = 3.1415926;
float phaseindex_U1 = pi;     //Start at pi radians so that the starting value of sin(phaseangle_U1) is 0.
float phaseindex_V1;
float phaseindex_W1;
float phaseindex_U2 = pi;     //Start at pi radians so that the starting value of sin(phaseangle_U1) is 0.
float phaseindex_V2;
float phaseindex_W2;
float phaseangle_U1; 
float phaseangle_V1;
...
float I_set_1;    //The user-determined current draw in mA. Power is automatically ramped to this value at start time.
float I_set_2; 
...
void Roll(){
	...
	phaseindex_U1 = phaseindex_U1 + wheel1_vel;    //This is fed into the sine function to vary the pwm duty cycle. rate is the step size for incrementing phaseindex_U1. Small = slow, big = fast, 0 = static.
	phaseindex_V1 = phaseindex_U1 + (2*pi/3);   //phaseindex_V1 is 1/3th phase ahead of phaseindex_U1
	phaseindex_W1 = phaseindex_V1 + (2*pi/3);   //phaseindex_W1 is 1/3th phase ahead of phaseindex_V1
	phaseindex_U2 = phaseindex_U2 + wheel2_vel;    //This is fed into the sine function to vary the pwm duty cycle. rate is the step size for incrementing phaseindex_U1. Small = slow, big = fast, 0 = static.
	phaseindex_V2 = phaseindex_U2 + (2*pi/3);   //phaseindex_V1 is 1/3th phase ahead of phaseindex_U1
	phaseindex_W2 = phaseindex_V2 + (2*pi/3);   //phaseindex_W1 is 1/3th phase ahead of phaseindex_V1


	if(phaseindex_U1 >= 8*pi) {   //Reset phaseindex_U1 once it completes 8*360o in phase. Could be just 360o, but this makes the code simpler for the torque ripple calibration mode.
		phaseindex_U1 = 0;
	}
	if(phaseindex_U2 >= 8*pi) {  
		phaseindex_U2 = 0;
	}

	phaseangle_U1 = phaseindex_U1;    //Replace with function that maps phaseindex into phaseangle according to the cogging correction profile.
	phaseangle_V1 = phaseindex_V1; 
	phaseangle_W1 = phaseindex_W1; 
	phaseangle_U2 = phaseindex_U2; 
	phaseangle_V2 = phaseindex_V2; 
	phaseangle_W2 = phaseindex_W2; 

	dutyU1 = (255/2)+(power_1*(sin(phaseangle_U1)+(.05*sin(3*phaseangle_U1))));   //The duty cycle varies with the sine function, which has output between -1 and 1. That is scaled by an amplitude variable, which effectively sets the motor power.This is all offset by half the maximum duty cycle so that the lowest instantaeous duty cycle is always positive.
	dutyV1 = (255/2)+(power_1*(sin(phaseangle_V1)+(.05*sin(3*phaseangle_V1))));
	dutyW1 = (255/2)+(power_1*(sin(phaseangle_W1)+(.05*sin(3*phaseangle_W1))));

	dutyU2 = (255/2)+(power_2*(sin(phaseangle_U2)+(.05*sin(3*phaseangle_U2))));
	dutyV2 = (255/2)+(power_2*(sin(phaseangle_V2)+(.05*sin(3*phaseangle_V2))));
	dutyW2 = (255/2)+(power_2*(sin(phaseangle_W2)+(.05*sin(3*phaseangle_W2))));
