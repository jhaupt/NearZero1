...
void PROMPT_I1Entry(){		
	...
	if (Iset1 > 5000){
		Serial.println(F("Maximum allowed current is 5A. CURRENT NOT CHANGED.\n"));
		Serial.println(F("\n"));
		exitflag = true;
		return;	
	}
...
void PROMPT_I2Entry(){			
	...
	if (Iset2 > 5000){
		Serial.println(F("Maximum current is 5A. CURRENT NOT CHANGED.\n"));
		Serial.println(F("\n"));
		exitflag = true;
		delay(2000);
		...
		Serial.println(F("CURRENT NOT CHANGED\n"));
		Serial.println(F("\n"));
		exitflag = true;		
		delay(2000);
		return;
...
void PROMPT_gain2Entry(){			
	...
	if (gain2 > 1000){
		Serial.println(F("Maximum gain is 1000. GAIN NOT CHANGED.\n"));
		Serial.println(F("\n"));
		exitflag = true;
		delay(2000);
		return;		
...
void PROMPT_tsphase2Entry(){			
	...
	if (tsphase2 > 6283){
		Serial.println(F("Maximum phase is 2pi (6283). PHASE NOT CHANGED.\n"));
		Serial.println(F("\n"));
		exitflag = true;
		delay(2000);
		return;		
...
void PROMPT_tscoeff2Entry(){			
	...
	if (tscoeff2 > 254){
		Serial.println(F("Maximum value is 254. AGGRESSIVENESS NOT CHANGED.\n"));
		Serial.println(F("\n"));
		exitflag = true;
		delay(2000);
		return;	
		...
		Serial.println(F("AGGRESSIVENESS NOT CHANGED"));
		Serial.println(F("\n"));
		exitflag = true;
		delay(2000);
		return;	
...
void PROMPT_accel2Entry(){			
	...
	if (accel2 > 10000){
		Serial.println(F("Maximum acceleration is 10,000. ACCELERATION NOT CHANGED.\n"));
		Serial.println(F("\n"));
		exitflag = true;
		delay(2000);
		return;		
	}
	else if (accel2 > 0 && accel2 <= 10000){
		//Don't do anything. This is just here so we can have the below "else" to detect an invalid entry.
	}
	else {
		Serial.println(F("ACCELERATION NOT CHANGED\n"));
		Serial.println(F("\n"));
		exitflag = true;		
		return;
	}
...
int SerialPrompt(){
	...
	if (rxchars[0] == '2'){	
		...
		Serial.println(F("3: Set command type to VELOCITY or POSITION"));
		Serial.println(F("4: Set velocity/position GAIN"));
		Serial.println(F("5: Set ACCELERATION"));
		Serial.println(F("6: Set waveform TORQUE SMOOTHING"));
		Serial.println(F("7: Display all present settings"));
		Serial.println(F(" "));
		Serial.println(F("m: Return to main menu"));
		//Serial.println("h: Help... what is all this?");
		Serial.println(F("\n\n>>"));
		...
		if (rxchars[0] == '0'){		
			...
			if (rxchars[0] == 'n' || rxchars[0] == 'N'){
				...
				Serial.print(F("CHANNEL 2 SENSOR SET TO 'NONE'"));
			}			
			else if (rxchars[0] == 'e' || rxchars[0] == 'E'){
		...
		//**1: Set current (Amperes) to FIXED or DYNAMIC
			...
			Serial.println(F("   f: Use FIXED current"));
			Serial.println(F("   d: Vary current DYNAMICALLY up to a maximum (encoder or hall sensor required)"));
			Serial.println(F("\n   (Or strike 'Enter' to leave as is)'")); 
			Serial.println(F("\n\n>>"));
			SerialReceiveLoop();
			if (rxchars[0] == 'f' || rxchars[0] == 'F'){
				...
				Serial.println(F("CURRENT TYPE NOT CHANGED"));
			}
			Serial.println(F("\n\n\n\n"));
			delay(2000);		
			goto ch2menu;
		...
		//**2: Set resting current (Amperes) for channel 2
			...
			Serial.println(F("   When in DYNAMIC current limiting mode this enforces a minimum current."));
			Serial.println(F("   Lowest allowed value is 1 (.001A)."));
			...
			Serial.print(F("CHANNEL 2 REST/MINIMUM CURRENT SET TO "));
			Serial.print(fholder);
			Serial.println(F("A"));
			Serial.println(F("\n\n\n\n"));
			delay(2000);		
		...
		//**6: Set waveform TORQUE SMOOTHING
		else if (rxchars[0] == '6'){
		...
		//**7: Display all present settings
		else if (rxchars[0] == '7'){
			CheckConfig();		//Read EEPROM
