...
int k = 0;		//general purpose counter for the roll functions
int m = 0;		//general purpose counter for the roll functions
...
void Roll_vel2(){
	...
	if (accel2 == 10000) {	//If accel=10000, this means the accel variable shouldn't even be used
		...
		if ((wheel2_vel - .0001) > wheel2_actualvel){		//the constant defines the deadzone	
			...
			if (currentmode2 == 0){	//If in fixed current mode...	
				power_2 = power_2_high;		//...set current to high
			}
		}
	...
	//CONSIDER HALL INPUT (IF TURNED ON)
	if (sensortype2 == 2){		//If the sensor type is HALL
		//power_2 = power_2_low;	//This will get reset immediately if not high enough
		if (phaseindex_2 > ((2*pi)/3 + 1) && phaseindex_2 < ((4*pi)/3 - 1)){
			if (digitalRead(enc2_pin3) == LOW){		//If this input hasn't come on yet
				//wheel2_actualvel = 0;		//Hold the phase index until the motor catches up
				if (currentmode2 == 1) {	//If in dynamic current mode
					j = (int) 10*fabs(phaseindex_2 - ((3*pi)/3));
					Serial.println(j);
					power_2 = power_2 + j;
					if (power_2 > power_2_high){	//...but only up to the max
				...
				if (currentmode2 == 1){
					power_2 = power_2 - 1;
					if (power_2 < power_2_low){
						power_2 = power_2_low;
					}
				}
			}	
		}
		if (phaseindex_2 > ((4*pi)/3 + 1) && phaseindex_2 < (2*pi - 1)){
			if (digitalRead(enc2_pin4) == LOW){		//If this input hasn't come on yet
				//wheel2_actualvel = 0;		//Hold the phase index until the motor catches up
				if (currentmode2 == 1) {	//If in dynamic current mode
					j = (int) 10*fabs(phaseindex_2 - ((5*pi)/3));
					Serial.println(j);
					power_2 = power_2 + j;
					if (power_2 > power_2_high){	//...but only up to the max
						power_2 = power_2_high;
		...
		if (phaseindex_2 > (0 + 1) && phaseindex_2 < ((2*pi)/3 - 1)){
			if (digitalRead(enc2_pin5) == LOW){		//If this input hasn't come on yet
				//wheel2_actualvel = 0;		//Hold the phase index until the motor catches up
				if (currentmode2 == 1) {	//If in dynamic current mode
					j = (int) 10*fabs(phaseindex_2 - (pi/3));
					Serial.println(j);
					power_2 = power_2 + j;
					if (power_2 > power_2_high){	//...but only up to the max
						power_2 = power_2_high;
