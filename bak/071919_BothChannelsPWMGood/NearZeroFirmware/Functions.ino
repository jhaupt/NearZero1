...
void CheckConfig(){
	...
	if (commandmode1 == 255){
		commandmode1 = 1;		//0 = VELOCITY control, 1 = POSITION control
	}
	//Read commandmode2
	...
	//Read sensphaseoffset2
	lhlf = EEPROM.read(addr_sensphaseoffset2l);  //read left half
	rhlf = EEPROM.read(addr_sensphaseoffset2r);  //read right half 
	sensphaseoffset2 = lhlf*100 + rhlf;    //Put the two halves back together
	if (sensphaseoffset2 > 7000){	//If the number is bigger than our maximum, assume that the initial current configuration was never done and reset Iset to a default value.
		sensphaseoffset2 = .7;
	}
	else {
		sensphaseoffset2 = sensphaseoffset2 / 1000;		//Convert floating point integer to decimal
	}
...
void DisplaySettings2(){
	...
	if (sensortype2 == 0){
		...
		Serial.print(F("   Hall phase offset: "));
		Serial.println(sensphaseoffset2);
	}	
...
void HallCal2(){
	...
	TorqueCal2();	//Roll the motor a bit to make sure the phase is registered to the actual position.
	sensphaseoffset2_meas = sensphaseoffset2;	
	digitalWrite(U2_En, HIGH);    //Enable each phase
	digitalWrite(V2_En, HIGH);
	digitalWrite(W2_En, HIGH);
	do {	//Iterate on measurement routine
		k++;
		Serial.print(F("Calibration iteration "));
		Serial.println(k);
		dummyphaseindex = 0;
		...
		do {	//ROLL FORWARDS
			...
			dummyphaseindex = dummyphaseindex + vel2;
			Roll_vel2();
			phaselag_runningav_f = phaselag_runningav_f + (phaselag2U+phaselag2V+phaselag2W)/3;		
		}
		while(dummyphaseindex < 14*pi);
		phaselag_runningav_f = phaselag_runningav_f/cnt;
		//Serial.println(phaselag_runningav_f);
		cnt = 0;
		vel2 = -.003;
		...
		do {	//ROLL BACKWARDS
			...
			dummyphaseindex = dummyphaseindex + vel2;
			Roll_vel2();
			phaselag_runningav_b = phaselag_runningav_b + (phaselag2U+phaselag2V+phaselag2W)/3;	
		}
		while(dummyphaseindex >= 0);
		phaselag_runningav_b = phaselag_runningav_b/cnt;
		//Serial.println(phaselag_runningav_b);
		sensphaseoffset2_meas = sensphaseoffset2_meas + (phaselag_runningav_f - phaselag_runningav_b);	//Update sensphaseoffset_meas
		sensphaseoffset2 = sensphaseoffset2_meas;
		Serial.print("Offset = ");
		Serial.println(sensphaseoffset2);
		Serial.print("f/b delta = ");	
		Serial.println(phaselag_runningav_f - phaselag_runningav_b);
		Serial.println(" ");
		if (k > 30){
			Serial.println(F("Something seems to be wrong. Check your connections, restart, and try again."));
			Serial.println(F("Another possibility is that the motor's hall sensors have too much gain, in which case"));
			Serial.println(F("the motor may not be usable for this type of operation. "));
			exitflag = true;
			break;
		}
		if (sensphaseoffset2_meas < 0){	//We never want it to be negative, so if it goes negative just make it some arbitrary postive number.
			Serial.println(F("Something seems to be wrong. Check your connections, restart, and try again."));
			Serial.println(F("Another possibility is that the motor's hall sensors have too much gain, in which case"));
			Serial.println(F("the motor may not be usable for this type of operation. "));
			exitflag = true;
	...
	while((fabs(phaselag_runningav_f - phaselag_runningav_b) > .03) || (k < 3));	//Only iterate until the error delta goes lower than some value, but do it at least 3 times.
	digitalWrite(U2_En, LOW);    
	digitalWrite(V2_En, LOW);
	digitalWrite(W2_En, LOW);
...
void ReadHall2(){
		if (digitalRead(enc2_pin5) == HIGH && enc2_pin5_flag == 0){
				...
				//if ((phaselag2U - phaselag2U_last) > 1){	//If the difference between this phaselage and the last is very big, discard it
				//	phaselag2U = phaselag2U_last;
				//}
				//phaselag2U_last = phaselag2U;
		...
		if (digitalRead(enc2_pin4) == HIGH && enc2_pin4_flag == 0){
				...
				//if ((phaselag2V - phaselag2V_last) > 1){	//If the difference between this phaselage and the last is very big, discard it
				//	phaselag2V = phaselag2V_last;
				//}
				//phaselag2V_last = phaselag2V;
		...
		if (digitalRead(enc2_pin3) == HIGH && enc2_pin3_flag == 0){
				...
				//if ((phaselag2W - phaselag2W_last) > 1){	//If the difference between this phaselage and the last is very big, discard it
				//	phaselag2W = phaselag2W_last;
				//}
				//phaselag2W_last = phaselag2W;
...
/*void PhaselagFloor2(){		//This function always takes the lowest phaselag
	if (phaselag2V < phaselag2W){
		if (phaselag2V < phaselag2U){
			bestphaselag2 = phaselag2V;
		}
		bestphaselag2 = phaselag2U;
	}
	else {
		bestphaselag2 = phaselag2W;
	}
}*/
	...
	if((digitalRead(mode_usb) == HIGH) && (digitalRead(mode_i2c) == HIGH) && (digitalRead(mode_config) == HIGH))  {     //If none of the mode pins are LOW, read the PWM inputs to control each motor's velocity independently.
		...
		if(pwm1active == true){     //Only get velocity data from this PWM channel if it was found to be connected above    
			if (commandmode1 == 0){	//if in velocity mode
				vel1 = (pwm1_t-1500)*.000006*gain1;	//Normal PWM servo pulsewidth goes from 800 to 2200ms. We subtract 1500ms, corresponding to the middle of that range, so that pulses in the range 800-1499 will have negative velocity, and 1501-2200 will be positive. 
			}
			else if (commandmode1 == 1){
				pos1 = (pwm1_t-1500)*.0005*gain1;
			}
		}
