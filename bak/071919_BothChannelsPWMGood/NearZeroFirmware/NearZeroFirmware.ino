...
#define addr_dir1 30
#define addr_dir2 31
#define addr_sensphaseoffset2l 32
#define addr_sensphaseoffset2r 33
...
float phaselag2U_last = 0;
float phaselag2V = 0;
float phaselag2V_last = 0;
float phaselag2W = 0;
float phaselag2W_last = 0;
float sensphaseoffset2;		//This gets determined empirically during a user-initiated self-calibration. Default set in CheckConfig();
float sensphasehalfwidth2 = pi/2;	//pi/2;
float sensphaseoffset2_meas;		//Holds as-measured sens phase offset. Auto-calibration runs faster if this is close to the final value to begin with.
float phaselag_runningav_f = 0;
float phaselag_runningav_b = 0;
float phaseangle_U1; 
float phaseangle_V1;
float phaseangle_W1;
float phaseangle_U2;
...
void Roll_vel2(){
	...
	if (sensortype2 == 2){
		...
		if (currentmode2 == 0) {	
			...
			//INCREMENT PHASE INDEX
			if (dir2 == 0) {	//If direction is NORMAL
				phaseindex2 = phaseindex2 + actualvel2;	//((20/(100*((phaselag2U+phaselag2V+phaselag2W)/3)))*actualvel2);    //This is fed into the sine function to vary the pwm duty cycle. rate is the step size for incrementing phaseindex_U1
			}
			else if (dir2 == 1) {	//If direction is REVERSE
				phaseindex2 = phaseindex2 - actualvel2;	//((20/(100*((phaselag2U+phaselag2V+phaselag2W)/3)))*actualvel2);
			}		
		}
		//IF WE'RE IN 'DYNAMIC' CURRENT MODE
		else if (currentmode2 == 1) {
			power2 = power2_low + ((int) 100*((phaselag2U+phaselag2V+phaselag2W)/3)) - 20;				
...
void Roll_pos1(){
	...
	//INCREMENT PHASE INDEX
	if ((posindexproxy1 - .2) > pos1){		//the constant defines the deadzone
		posindexproxy1 = posindexproxy1 - servovel1; 
		phaseindex1 = phaseindex1 - servovel1; 
		power1 = power1_high;
		j = 0;
	}
	else if ((posindexproxy1 + .2) < pos1){
		posindexproxy1 = posindexproxy1 + servovel1;
		phaseindex1 = phaseindex1 + servovel1;
		power1 = power1_high;
		j = 0;
