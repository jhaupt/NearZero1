...
void PROMPT_sensphaseoffset2Entry(){
	SerialReceiveLoop();
	iholder = atoi(rxchars);	//Convert rxchars to an integer 
	if (iholder > 6280){
		CheckConfig();
		Serial.print(F("Maximum hall phase offset is 2pi (6.28). OFFSET LEFT AT "));
	...
	else if (iholder >= 0 && iholder <= 6280){} //Don't do anything. This is just here so we can have the below "else" to detect an invalid entry.
	else {
		CheckConfig();
		Serial.print(F("Invalid entry. OFFSET LEFT AT "));
		Serial.println(sensphaseoffset2);
		Serial.println(F("\n\n"));
		exitflag = true;		
		return;
	...
	EEPROM.write(addr_sensphaseoffset2l, lhlf);   
	EEPROM.write(addr_sensphaseoffset2r, rhlf);	
}
	...
	if (rxchars[0] == '2'){	
		...
		if (rxchars[0] == '0'){		
			...
			if (rxchars[0] == 'n' || rxchars[0] == 'N'){
				...
				hallmenu2:	
				exitflag = false;
				Serial.println(F("\nThe hall sensor offset must be entered for each new make/model of motor used:"));
				Serial.println(F("   1: Perform self-calibration (motor must be free and unloaded)"));
				Serial.println(F("   2: Enter previously-measured offset"));
				Serial.println(F("   3: Leave as is"));
				Serial.println(F("   4: Go back to previous menu without changing sensor type")); 
				Serial.println(F("\n\n>>"));
				SerialReceiveLoop();
				if (rxchars[0] == '1'){
					Serial.println(F("Measuring hall sensor phase offset (motor should be turning)..."));	
					HallCal2();	
					if (exitflag == true){
						Serial.println(F("\nHALL MODE NOT SET"));
						delay(2000);
					...
					Serial.print(F("\nMeasured hall phase offset for channel 2: "));
					Serial.println(sensphaseoffset2_meas);
					Serial.println(F("\nUse this value and write to EEPROM? (Y/n)"));
					Serial.println(F("\n\n>>"));	
					SerialReceiveLoop();
					if (rxchars[0] == 'n' || rxchars[0] == 'N'){
						CheckConfig();		//Needed to reset sensphaseoffset back to original value.
						...
						fholder = sensphaseoffset2 *1000;	//Temporarily convert to integer to store in EEPROM
						...
						EEPROM.write(addr_sensphaseoffset2l, lhlf);   //write the left two digits of iholder to the address location given by addr_.
						EEPROM.write(addr_sensphaseoffset2r, rhlf);	//etc.		
						Serial.print(F("CHANNEL 2 SUCCESSFULLY CONFIGURED WITH HALL SENSOR"));						
					}
				}
				else if (rxchars[0] == '2'){
					Serial.println(F("Enter hall sensor phase offset as an integer in thousandths (e.g. enter \"700\" for 0.70)"));	
					Serial.println(F("   or strike 'Enter' to leave as is:")); 
					Serial.println(F("\n\n>>"));
					PROMPT_sensphaseoffset2Entry();
					if (exitflag == true){
						delay(2000);
						goto hallmenu2;		//Yes that's a goto. FRACK modern programming sensibilities. You know what I mean?
					}
					CheckConfig();
					Serial.print(F("CHANNEL 2 HALL PHASE OFFSET SET TO: "));
					Serial.println(sensphaseoffset2);
					EEPROM.write(addr_sensortype2, 2);	//2 = hall
				}
				else if (rxchars[0] == '3'){
					Serial.print(F("HALL PHASE OFFSET LEFT AT "));
					Serial.print(sensphaseoffset2);
					EEPROM.write(addr_sensortype2, 2);	//2 = hall
				}
				else if (rxchars[0] == '4'){
					goto sensormenu2;
