...
void Roll(){
	...
	if(phaseindex_U1 >= 14*pi) {   //Reset phaseindex_U1 once it completes 10*180o in phase. Could be just 2*180o, but this makes the code simpler for the torque ripple calibration mode.
		phaseindex_U1 = 0;
	}
	if(phaseindex_U2 >= 14*pi) {  
		phaseindex_U2 = 0;
	}

	if (cogprof_1 == 0){	//If the anti-coffing profile is set to 0...
		phaseangle_U1 = phaseindex_U1;    //Just map phaseindex to phaseangle 1 to 1.
	...
	if (cogprof_1 == 1){	//If the anti-coffing profile is set to 1...
