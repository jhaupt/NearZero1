...
void HallCal2(){
	...
	do {	
		...
		if (phase2_T_deviation < phase2_T_deviation_last) {
			c2TrigSequenceStore = cnt;	
		}
		phase2_T_deviation_last = phase2_T_deviation;
		cnt++;
	}
	while(cnt <= 6);
...
void ReadHall2(){		//!!!!!!!!!Similar for reading encoder!!!!!!!!!!

	//Reset mTrig2 every phase cycle
	if (phaseindex2 >= ((2*pi)-.05) || phaseindex2 <= .05){	
		c2Trig1 = millis();
	}
	if (phaseindex2 >= (((2*pi)/3)-.05) && phaseindex2 <= (((2*pi)/3)+.05)){
		c2Trig2 = millis();
	}
	if (phaseindex2 >= (((4*pi)/3)-.05) && phaseindex2 <= (((4*pi)/3)+.05)){
		c2Trig3 = millis();
	}
