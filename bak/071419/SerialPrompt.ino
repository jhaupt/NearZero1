...
void PROMPT_maxI2Entry(){			
	...
	if (maxIset2 > 5000){
		...
		Serial.print(F("Maximum current is 5A. CURRENT LEFT AT "));
...
int SerialPrompt(){
	...
	if (rxchars[0] == '2'){	
		...
		//**1: Set current (Amperes) to FIXED or DYNAMIC
			...
			Serial.println(F("\n   1: Use FIXED power"));
			Serial.println(F("   2: Use FIXED power and use lower RESTING power when commanded velocity is 0"));
			Serial.println(F("   3: Vary power DYNAMICALLY based on mechanical resistance (encoder or hall sensor required)"));
			Serial.println(F("\n   (Or strike 'Enter' to leave as is)")); 
			Serial.println(F("\n\n>>"));
			SerialReceiveLoop();
			if (rxchars[0] == '1'){
