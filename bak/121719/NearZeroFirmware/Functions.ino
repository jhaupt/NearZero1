void CheckConfig(){
	...
	if (commandmode1 == 255){
		commandmode1 = 0;		//0 = VELOCITY control, 1 = POSITION control		ch1 on vel and ch2 on pos works
	}
	//Read commandmode2
	commandmode2 = EEPROM.read(addr_commandmode2);
	if (commandmode2 == 255){
		commandmode2 = 0;		//1 = VELOCITY control, 1 = POSITION control
	}
	//Read sensortype1
...
void DisplaySettings(){
	...
	if (commandtopic == 1){
		...
		if (commandmode1 == 0){
			Serial.println(F("   ROS and PWM command mode: VELOCITY"));
		}
		else if (commandmode1 == 1){
