...
/*
Todo: 
-Replace .03 with servovel and add necessary meny entry and config stuff
-Transfer channel 2 code over to channel 1
-Add encoder/hall support and add publisher for the /odom topic. Reference http://wiki.ros.org/navigation/Tutorials/RobotSetup/Odom
...
float tsphase2;
int servovel1;	//Sets gain of position following velocity (sets servo aggressiveness)
int servovel2;
...
void Roll_vel2(){
	...
	phaseindex_2 = phaseindex_2 + wheel2_vel;    //This is fed into the sine function to vary the pwm duty cycle. rate is the step size for incrementing phaseindex_U1. 
	
	if (wheel2_vel == 0){	//if the wheel isn't moving (is resting), set the power low as determined by the config settings
		power_2 == power_2_low;
	}
	else {
		power_2 = power_2_high;	//If the wheel is moving, use the high power as determined by by the config settings 
	}
	...
	if(fabs(phaseindex_2) >= 100*pi) {   //Reset phaseindex_U1 once it completes 2*180o in phase.
		phaseindex_2 = 0;
	}
...
void Roll_pos2(){
	...
	wheel2_ServoVel = .03*(fabs(phaseindex_2 - wheel2_pos));		//replace .03 with servovel as user-config value
