...
float wheel1_cmd = 0;    //sets the velocity (positive or negative) or position. This is what increments the phaseangle for each phase.
float wheel2_cmd = 0;
...
void Roll(){
	...
	if (commandmode1 == 0){		//if in velocity mode
		phaseindex_1 = phaseindex_1 + wheel1_cmd;    //This is fed into the sine function to vary the pwm duty cycle. rate is the step size for incrementing phaseindex_U1. 
	}
	else if (commandmode1 == 1){	//if in position mode	
		phaseindex_1 = wheel1_cmd;
	}

	if (commandmode2 == 0){		//if in velocity mode
		phaseindex_2 = phaseindex_2 + wheel2_cmd;    //This is fed into the sine function to vary the pwm duty cycle. rate is the step size for incrementing phaseindex_U1. 
	}
	else if (commandmode2 == 1){	//if in position mode	
		phaseindex_2 = wheel2_cmd;
	}
