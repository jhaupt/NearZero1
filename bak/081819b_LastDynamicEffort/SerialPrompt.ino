...
void PROMPT_c2TrigOffsetEntry(){
	...
	if (iholder >= 0 && iholder <= 10000){} //Don't do anything. This is just here so we can have the below "else" to detect an invalid entry.
		...
		CheckConfig();
		Serial.print(F("Invalid entry. c2TrigOffset LEFT AS "));
	...
	EEPROM.write(addr_c2TrigOffsetl, lhlf);   
	EEPROM.write(addr_c2TrigOffsetr, rhlf);	
}

void PROMPT_c2vTcalEntry(){
	SerialReceiveLoop();
	iholder = atoi(rxchars);	//Convert rxchars to an integer 
	if (iholder >= 0 && iholder <= 10000){} //Don't do anything. This is just here so we can have the below "else" to detect an invalid entry.
	else {
		CheckConfig();
		Serial.print(F("Invalid entry. c2vTcal LEFT AS "));
	...
	EEPROM.write(addr_c2vTcall, lhlf);   
	EEPROM.write(addr_c2vTcalr, rhlf);	
}
...
int SerialPrompt(){
	...
	Serial.println(F("          or RECONNECTING TO THIS TERMINAL\n"));
		...
		if (rxchars[0] == '0'){		
			...
			if (rxchars[0] == 'n' || rxchars[0] == 'N'){
				...
				if (rxchars[0] == '1'){
					...
					Serial.print(F("   c2TrigOffset = "));
					Serial.println(c2TrigOffset);
					Serial.print(F("   c2TrigSequence = "));
					Serial.println(c2TrigSequence);
					Serial.print(F("   c2vTcal = "));
					Serial.println(c2vTcal);
					...
					if (rxchars[0] == 'n' || rxchars[0] == 'N'){
						...
						if (c2TrigOffset < 100){		//If it's a 1 or 2 digit number
							lhlf = 0;	//The left half of the four digit number is 0
							rhlf = c2TrigOffset;		//And the right half is just the number
						}
						else {		//If it's a 3 or 4 digit number
							lhlf = floor(c2TrigOffset/100);		//The left half is the leading digit of a 3 digit number or leading 2 digits of a 4 digit number.
							rhlf = c2TrigOffset - (lhlf*100);	//The right half is the trailing two digits of the number.
						}
						EEPROM.write(addr_c2TrigOffsetl, lhlf);   //write the left two digits of iholder to the address location given by addr_.
						EEPROM.write(addr_c2TrigOffsetr, rhlf);	//etc.	
						if (c2vTcal < 100){		//If it's a 1 or 2 digit number
							lhlf = 0;	//The left half of the four digit number is 0
							rhlf = c2vTcal;		//And the right half is just the number
						}
						else {		//If it's a 3 or 4 digit number
							lhlf = floor(c2vTcal/100);		//The left half is the leading digit of a 3 digit number or leading 2 digits of a 4 digit number.
							rhlf = c2vTcal - (lhlf*100);	//The right half is the trailing two digits of the number.
						}
						EEPROM.write(addr_c2vTcall, lhlf);   //write the left two digits of iholder to the address location given by addr_.
						EEPROM.write(addr_c2vTcalr, rhlf);	//etc.		
						EEPROM.write(addr_c2TrigSequence, c2TrigSequence);	//etc.		
						Serial.print(F("CHANNEL 2 SUCCESSFULLY CONFIGURED WITH HALL SENSOR"));						
					}
				}
				else if (rxchars[0] == '2'){
					Serial.println(F("Enter c2TrigOffset calibration value"));	
					Serial.println(F("   or strike 'Enter' to leave as is:")); 
					Serial.println(F("\n\n>>"));
					PROMPT_c2TrigOffsetEntry();
					if (exitflag == true){
						delay(2000);
						goto hallmenu2;		//Yes that's a goto. FRACK modern programming sensibilities. You know what I mean?
					}
					CheckConfig();
					Serial.print(F("c2TrigOffset SET TO "));
					Serial.println(c2TrigOffset);
					Serial.println(F("\n "));

					Serial.println(F("Enter c2vTcal calibration value"));	
					Serial.println(F("   or strike 'Enter' to leave as is:")); 
					Serial.println(F("\n\n>>"));
					PROMPT_c2vTcalEntry();
					if (exitflag == true){
					...
					CheckConfig();
					Serial.print(F("c2vTcal SET TO "));
					Serial.print(c2vTcal);
					Serial.println(F("\n "));
					...
					Serial.print(F("c2TrigOffset LEFT AS "));
					Serial.println(c2TrigOffset);
					Serial.print(F("c2vTcal LEFT AS "));
					Serial.println(c2vTcal);
