...
*
* If using ROS, after roscore is started, do $rosrun rosserial_python serial_node.py /dev/ttyUSB[X] on the machine that the NearZero board is attached to, which will let NearZero interract with ROS.
...
void setup(){
	...
	if (digitalRead(mode_i2c) == LOW){	//This has to be the last thing that happens in setup() for some reason
		...
		Wire.onReceive(receiveEvent); // An ISR: if I2C data comes in go to the receiveEvent() function to handle it.
		Serial.begin(9600);
	}
}
