...
void PROMPT_I2CAddressEntry(){
	SerialReceiveLoop();
	i2caddress = atoi(rxchars);	//Convert rxchars to an integer 
	if (tscoeff1 > 254){
		CheckConfig();
...
int SerialPrompt(){
	...
	Serial.println(F("4: USB/ROS Configuration"));
	Serial.println(F("5: List current settings"));
	Serial.println(F("6: Reset factory defaults"));
	...
	if (rxchars[0] == '1'){	
		...
		Serial.println(F("5: Set position/servo ACCELERATION")); 
		Serial.println(F("6: Set maximum position/servo SLEW RATE"));
		Serial.println(F("7: Set PWM command GAIN"));
		Serial.println(F("8: Set PWM center OFFSET"));
		...
		//**Set PWM command type to VELOCITY or POSITION
			...
			Serial.println(F("Set channel 1 to interpret ROS, PWM, or I2C input as either velocity, position, or servo commands:"));
	...
	if (rxchars[0] == '2'){	
		...
		//**Set PWM command type to VELOCITY or POSITION
			...
			Serial.println(F("Set channel 2 to interpret ROS, PWM, or I2C input as either velocity, position, or servo commands:"));
			Serial.println(F("   v: VELOCITY"));
			Serial.println(F("   p: POSITION"));
			Serial.println(F("   s: SERVO (same as POSITION but with closed-loop control) -- requires encoder"));
			Serial.println(F("\n   (Or strike 'Enter' to leave as is)")); 
	...
	if (rxchars[0] == '4'){
		rosmenu:
		Serial.println(F("USB/ROS configuration:"));
