...
Todo: 
-Have these ROS config options: 
	a. Subscribe to cmdvel directly with encoder ticks going out on a new topic. Explain in manual that user would have to figure out how to convert this to odom and tf on the ROS side.
	b. Subscribe to left-side and right-side velocity messages as generated by a custom hardware interface for diff_drive_controller. The same hardware interface would also an encoder tick topic (same topic as (a)).
	c. Same as (b) but using a different ros_control controller and hardware interface intended for servo/joint control. 
	General note on this: 
		-The topics between the hardware interface and NearZero might be able to be the same for (b) and (c).
		-Change menu wording so that command mode for PWM vs ROS is clearly separated. ROS should be on main menu and PWM should remain channel-specific.
		-Still okay to publish idraw directly from NearZero, I think
...
#define addr_rostopic1 37
#define addr_rostopic2 38
#define addr_enctopic1 39
#define addr_enctopic2 40
...
int dir2;
int rostopic1;		//0 = /joint_commands, 1 = /cmd_vel
int rostopic2;
int wheelbasescale;
int wheeldiamscale;
...
char enctopic2[] = "nzticks1";	//define this to be 1 character longer than shown here to accomodate double-digit postfixes. 
...
ros::Publisher pub_2(enctopic2, &ticks_msg);	
