...
int SerialPrompt(){
	...
	if (rxchars[0] == '1'){	
		...
		Serial.println(F("0: Set positior sensor to NONE, ENCODER, or HALL"));
		Serial.println(F("1: Set moving and resting current (Amperes)"));
		Serial.println(F("2: Set resting current (Amperes)"));
		Serial.println(F("3: Set command type to VELOCITY or POSITION"));
		Serial.println(F("4: Set PWM velocity/position GAIN"));
	...
	if (rxchars[0] == '2'){	
		...
		Serial.println(F("3: Set ROS topics and sensor type"));
		...
		Serial.println(F("6: Set PWM velocity/position GAIN"));
		...
		//**Set ROS configuration
			...
			Serial.println(F("1:  Set sensor type (NONE, ENCODER, or HALL)"));
			Serial.println(F("2:  Set topic for publishing hall/encoder ticks"));
			Serial.println(F("3:  Set topic subscription for command input"));
			Serial.println(F("\n   (Or strike 'Enter' to leave as is)")); 
			Serial.println(F("\n\n>>"));
			SerialReceiveLoop();
			if (rxchars[0] == '1'){		
			...
			if (rxchars[0] == '2'){		
				...
				Serial.println(F("Enter a value for N or strike 'Enter' to leave as is:"));
				Serial.println(F("\n\n>>"));	
				SerialReceiveLoop();
				iholder = atoi(rxchars);
				if (iholder >= 1 && iholder <= 9){				
					EEPROM.write(addr_enctopic2, iholder);
					enctopic2[7] = rxchars[0];
					Serial.print(F("ENCODER/HALL TOPIC SET TO /"));
					Serial.println(enctopic2);
				}			
				else{
			...
			if (rxchars[0] == '3'){		
				Serial.println(F("Configure channel 2 ROS topic subscription:"));
				Serial.println(F("1:  Subscribe to /joint_commands topic"));
				...
				if (rxchars[0] == '1'){
					rostopic2 = 0;
					EEPROM.write(addr_rostopic2, rostopic2);	//0 = joint_command
					if (commandmode2 == 0){
						Serial.print(F("CHANNEL 2 SUBSCRIBED TO '/joint_commands' WITH VELOCITY INPUT"));
					}
					else if (commandmode2 == 1){
						Serial.print(F("CHANNEL 2 SUBSCRIBED TO '/joint_commands' WITH POSITION INPUT"));
					}
					else {
						Serial.print(F("CHANNEL 2 ROS TOPIC NOT CHANGED"));
						Serial.println(F("\n\n\n\n"));
						delay(2000);		
						goto ch2menu;
					}
					/*Serial.println(F("\n"));
					//Serial.println(F("Enter a name for this joint (use a unique string like 'joint5' or 'right_wheel'):")); 
					Serial.println(F("Enter the array index number for this joint (or strike 'Enter' to leave as is):")); 
					Serial.println(F("")); 	
					Serial.println(F("What does this mean? In the JointState message that's broadcast over the /joint_commands topic,"));
					Serial.println(F("an array of velocities or positions is provided where each element of the 'velocity' or 'position'"));
					Serial.println(F("array is intended for a separate joint (motor channel). There's also a 'name' array so that each "));
					Serial.println(F("joint has a user-defined, easily readible name, BUT the NearZero board doesn't recognize those names."));
					Serial.println(F("Therefore, it's necessary to associate each NearZero channel with the intended array index."));
					Serial.println(F("E.g.: If we want this channel to be controlled by a joint named 'left_wheel', and 'left_wheel' is"));
					Serial.println(F("the first joint in the 'name' array, we would want to set the array index to '0'. Note that there"));
					Serial.println(F("can be any number of joints and any number of NearZero boards can be used to control the joint hardware."));
					Serial.println(F("\n\n>>"));
					SerialReceiveLoop();
					rosindex2 = atoi(rxchars);	//Convert rxchars to an integer
					if (rosindex2 >= 0 && rosindex2 <=254){
						EEPROM.write(addr_rosindex2, rosindex2);	
						Serial.print(F("CHANNEL 2 JOINT INDEX SET TO '"));
						Serial.print(rosindex2);
						Serial.print(F("'."));
					}
					else {
						CheckConfig();
						Serial.print(F("CHANNEL 2 JOINT INDEX LEFT AT '"));
						Serial.print(rosindex2);
						Serial.print(F("'."));
					}*/
					Serial.println(F("\n\n\n\n"));
					...
					Serial.println(F("Subscribe to /cmd_vel for differential drive of channels 1 and 2? This will apply to BOTH channels. (y/N)")); 
					/*Serial.println(F(" "));
					Serial.println(F("This is not really preferred as it forces the NearZero hardware to do the differential drive"));
					Serial.println(F("calculations on-board, which is awkward because the configuration parameters for wheel diameter"));
					Serial.println(F("and base width then have to be handled via this menu instead of the convenient ROS configuration"));
					Serial.println(F("files, where they belong. What's more, the NearZero hardware does *not* publish to /odom and /tf"));
					Serial.println(F("directly as these require particuarly resource-intensive calculations. Instead, it publishes raw"));
					Serial.println(F("encoder (or hall) ticks on the /joint_states topic and depends on ROS-side software (either the NearZero ROS"));
					Serial.println(F("package or your own package) to ingest the ticks and publish odometry data to /odom and /tf."));
					Serial.println(F("So, considering a ROS package is needed to handle the odometry anyway, it's sensible to also let it"));
					Serial.println(F("subscribe to /cmd_vel and calculate the individual motor velocities as informed by the config file. It then"));
					Serial.println(F("publishes to /joint_command for each motor using the join names defined  with this menu system and"));
					Serial.println(F("in the config file."));
					Serial.println(F(" "));
					Serial.println(F("Really subscribe to /cmd_vel? This will apply to BOTH channels. (y/N)"));*/
					Serial.println(F("\n\n>>"));	
					SerialReceiveLoop();
					if (rxchars[0] == 'Y' || rxchars[0] == 'y'){
						rostopic2 = 1;
						//EEPROM.write(addr_rostopic1, rostopic1);	//1 = cmd_vel
						EEPROM.write(addr_rostopic2, rostopic2);	//1 = cmd_vel
						Serial.print(F("CHANNELS 1 & 2 ARE NOW SUBSCRIBED TO '/cmd_vel'"));
						Serial.println(F("\n\n"));
					...
					Serial.println(F("CHANNEL 2 ROS SETTINGS NOT CHANGED"));
				}			
				Serial.println(F("\n\n\n\n"));
				delay(2000);		
				goto ch2menu;
