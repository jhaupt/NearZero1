...
void CheckConfig(){
	...
	if (maxIset2 > 5000){	//If the number is bigger than our maximum, assume that the initial current configuration was never done and reset Iset to a default value.
		maxIset2 = 2000;
	}
	//Read minIset1
...
int SetPower(){
	...
	if (power_2 == 0){
		...
		Serial.print(I_A);	
		Serial.println(F("A (as measured)\n"));
		//Serial.print(F("\nDiagnostic output: power_2_high = "));
