...
void Roll_vel2(){
	...
	//IF WE'RE IN 'FIXED' CURRENT MODE WITH HALL SENSORS
	if (currentmode2 = 0) {			//IF WE'RE IN 'FIXED' CURRENT MODE
		if (sensortype2 == 2) {		//IF A HALL SENSOR IS PRESENT
	...
	//IF WE'RE IN 'DYNAMIC' CURRENT MODE
		...
		if (digitalRead(enc2_pin5) == HIGH && enc2_pin5_flag == 0){
				...
				if (wheel2_actualvel >= 0) { //If velocity is positive
					phaselag2U = phaseindex_2 - 2.2; 
					if (phaselag2U < 0) {phaselag2U = 0;}
				}
				else { //If velocity is negative
					phaselag2U = fabs(phaseindex_2 - 5.3); 
				}
		...
		if (digitalRead(enc2_pin4) == HIGH && enc2_pin4_flag == 0){
				enc2_pin4_flag = 1;
				if (wheel2_actualvel >= 0) { //If velocity is positive
					phaselag2V = phaseindex_2 - 4.6;	//offset derived empirically	 
					if (phaselag2V < 0) {phaselag2V = 0;}
				}
				else { //If velocity is negative
					phaselag2V = fabs(phaseindex_2 - 1.0); 	////offset derived empirically
				}
		...
		if (digitalRead(enc2_pin3) == HIGH && enc2_pin3_flag == 0){
				enc2_pin3_flag = 1;
				if (wheel2_actualvel >= 0) { //If velocity is positive
					phaselag2W = phaseindex_2 - 0.3;	//offset derived empirically	 
					if (phaselag2W < 0) {phaselag2W = 0;}
				}
				else { //If velocity is negative
					phaselag2W = fabs(phaseindex_2 - 3.2); 	////offset derived empirically
				}
