...
void Roll(){
	...
	if (cogprof_1 == 0){	//If the anti-coffing profile is set to 0, just map phaseindex to phaseangle 1 with the necessary 1/3 rotation offsets for each phaseangle.
		phaseangle_U1 = phaseindex_1; 
		phaseangle_V1 = phaseindex_1 + (2*pi/3); 
		phaseangle_W1 = phaseindex_1 + (4*pi/3); 
		phaseangle_U2 = phaseindex_2; 
		phaseangle_V2 = phaseindex_2 + (2*pi/3); 
		phaseangle_W2 = phaseindex_2 + (4*pi/3); 
	}
	if (cogprof_1 == 1){	//If the anti-coffing profile is set to 1, make the phase angle be the phase index plus a correction which is a function of the phase index. The fuction is a fourier series.
		phaseangle_U1 = phaseindex_1 + 1*sin(phaseindex_1) + 1*sin(2*phaseindex_1);  //REPLACE the sin coefficients with user-editable variables.
		phaseangle_V1 = phaseindex_1 + (2*pi/3); 
		phaseangle_W1 = phaseindex_1 + (4*pi/3); 
		phaseangle_U2 = phaseindex_2 + 1*sin(phaseindex_2) + 1*sin(2*phaseindex_2);
		phaseangle_V2 = phaseindex_2 + (2*pi/3); 
		phaseangle_W2 = phaseindex_2 + (4*pi/3); 
	}
