...
/*
Todo: 
-Fix fixed-current/Hall-on mode
-Move new functionality to POS mode
...
#define addr_Iset2l 2
#define addr_Iset2r 3
...
float phaselag2 = 0;		//Used in dynamic current mode
float phaseangle_U1; 
float phaseangle_V1;
float phaseangle_W1;
float phaseangle_U2;
...
/*
	...
	Serial.print("pin 3 = ");
	Serial.println(digitalRead(enc2_pin3));
	Serial.print("pin 4 = ");
	Serial.println(digitalRead(enc2_pin4));
	Serial.print("pin 5 = ");
	Serial.println(digitalRead(enc2_pin5));*/


	//CONSIDER HALL INPUT (IF TURNED ON)
	if (currentmode2 = 0) {		//IF WE'RE IN 'FIXED' CURRENT MODE
	...
	Serial.print("current mode1 = ");
	Serial.println(currentmode1);
	Serial.print("current mode2 = ");
	Serial.println(currentmode2);
	Serial.print("sensor type = ");
	Serial.println(sensortype2);
	Serial.print("tscoeff = ");
	Serial.println(tscoeff2);

	if (currentmode2 == 1 && sensortype2 == 2){		//IF THE SENSOR TYPE IS 'HALL' AND THE CURRENT MODE IS 'DYNAMIC'
		if (digitalRead(enc2_pin3) == HIGH && enc2_pin3_flag == 0){
				enc2_pin3_flag = 1;
				phaselag2 = (-1*fabs(phaseindex_2 - ((2*pi)/3+1.5)))+1.5; 
				//Serial.print("phaselag = ");
				//Serial.println(phaselag2);
		}
		else if (digitalRead(enc2_pin3) == LOW && enc2_pin3_flag == 1){
			enc2_pin3_flag = 0;
		}
		power_2 = power_2_low + ((int) 10*phaselag2);
